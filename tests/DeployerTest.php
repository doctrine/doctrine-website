...
class DeployerTest extends TestCase
{
    ...
    public function testDeployStaging() : void
    {
        ...
        $this->processFactory->expects(self::at(2))
            ->method('run')
            ->with('cd /data/doctrine-website-staging && ./bin/console build-website-data && ./bin/console build-docs --api --sync-git && ./bin/console build-website /data/doctrine-website-build-staging --env=staging --publish')
            ->willReturn($process);
